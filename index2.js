...
// //post by a specific id handling 
// app
//   .route("/api/posts/:id")
//   .get((req, res, next) => {
...
//       if (p.id == req.params.id) { //looping over each post until we find a match
//         for (const key in req.body) { //found the post we need to update it using a for in - use to loop over the keys of the object
//             //find the post by index - if the current post matched the id for the parameter is updates the post 
//             //with the new key - assign the original key to the req.body value at that key 
//             //doing all this inside the .find - find method wants you to return true 
//             //find it, change it, return true 
...
//   .delete((req, res, next) => {
//     // The DELETE request route simply removes a resource.
//     const post = posts.find((p, i) => { //index provided by the find method - 
//         //if we find the post splice out the part we want to delete it and return true 
